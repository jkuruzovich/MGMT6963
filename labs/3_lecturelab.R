...
#DATASTRUCTURE-> Vector, Matrix, DataFrame
#Aggregation
#Joins

#Important Functions
# c =Concatentate
# cbind = Combine objects
...
# We can create a vector using the 
# concatenate command (c)

# Let's say we want to capture the ages for 
# 4 students.
ages<-c(18,19,18,23)
...
names.length.gt4 #what type of vector is returned?
#or
names.length.gt4b<-names.length > 4
names.length.gt4b #what type of vector is returned?

#look at select we can do when combining vectors
names[female] #what type of vector is returned and what content?
names[names.length.gt4] #what type of vector is returned and what content?
...
getwd()
setwd("~/Dropbox/30_classes/analytics/2013_fall/labs/rlab1")
list.files()
...
as.Date(datez, "%m/%d/%y")
#Look at the help (?strptime) to see why capital Y
